...
{
  static constexpr bool value = internal::is_integral<T>::value || std::is_enum<T>::value;
};

// true if both types are not valid index types
template<typename RowIndices, typename ColIndices>
struct valid_indexed_view_overload {
  static constexpr bool value = !(internal::is_valid_index_type<RowIndices>::value && internal::is_valid_index_type<ColIndices>::value);
};
...
{
  static constexpr int value = AlignmentBytes;
};
...
struct compute_default_alignment_helper<ArrayBytes, AlignmentBytes, false, true> // Try-half
{
  ...
  static constexpr int value = compute_default_alignment_helper<ArrayBytes, AlignmentBytes/2>::value;
};
#else
...
template<typename T, int Size> struct compute_default_alignment {
  static constexpr int value = compute_default_alignment_helper<Size*sizeof(T),EIGEN_MAX_STATIC_ALIGN_BYTES>::value;
};

template<typename T> struct compute_default_alignment<T,Dynamic> {
  static constexpr int value = EIGEN_MAX_ALIGN_BYTES;
};
...
{
    static constexpr int row_major_bit = Options&RowMajor ? RowMajorBit : 0;
  public:
    // FIXME currently we still have to handle DirectAccessBit at the expression level to handle DenseCoeffsBase<>
    // and then propagate this information to the evaluator's flags.
    // However, I (Gael) think that DirectAccessBit should only matter at the evaluation stage.
    static constexpr int ret = DirectAccessBit | LvalueBit | NestByRefBit | row_major_bit;
};

template<int Rows_, int Cols_> struct size_at_compile_time
{
  static constexpr int ret = (Rows_==Dynamic || Cols_==Dynamic) ? Dynamic : Rows_ * Cols_;
};

template<typename XprType> struct size_of_xpr_at_compile_time
{
  static constexpr int ret = size_at_compile_time<traits<XprType>::RowsAtCompileTime,traits<XprType>::ColsAtCompileTime>::ret;
};
...
template<typename T, int n, typename PlainObject = typename plain_object_eval<T>::type> struct nested_eval
{
  ...
  static constexpr bool
    Evaluate = (evaluator<T>::Flags & EvalBeforeNestingBit) || (CostEval < CostNoEval);
...
template <typename LhsKind, typename RhsKind, int LhsOrder, int RhsOrder> struct cwise_promote_storage_order {
  static constexpr int value = LhsOrder;
};

template <typename LhsKind, int LhsOrder, int RhsOrder>   struct cwise_promote_storage_order<LhsKind,Sparse,LhsOrder,RhsOrder>                { static constexpr int value = RhsOrder; };
template <typename RhsKind, int LhsOrder, int RhsOrder>   struct cwise_promote_storage_order<Sparse,RhsKind,LhsOrder,RhsOrder>                { static constexpr int value = LhsOrder; };
template <int Order>                                      struct cwise_promote_storage_order<Sparse,Sparse,Order,Order>                       { static constexpr int value = Order; };
...
{
  static constexpr bool
      value = (!is_const<ExpressionType>::value) &&
...
struct possibly_same_dense {
  static constexpr bool value = has_direct_access<T1>::ret && has_direct_access<T2>::ret && is_same<typename T1::Scalar,typename T2::Scalar>::value;
};
...
struct scalar_div_cost {
  static constexpr int value = 8*NumTraits<T>::MulCost;
};
...
struct scalar_div_cost<signed long,Vectorized, std::conditional_t<sizeof(long)==8,void,false_type>> { static constexpr int value = 24; };
template<bool Vectorized>
struct scalar_div_cost<unsigned long,Vectorized, std::conditional_t<sizeof(long)==8,void,false_type>> { static constexpr int value = 21; };
