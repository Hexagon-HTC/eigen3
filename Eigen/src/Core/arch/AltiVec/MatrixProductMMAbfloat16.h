...
EIGEN_STRONG_INLINE void pgerMMAbfloat16(__vector_quad* acc, const Packet8bf& a, const Packet8bf& b, int maskX, int maskY)
{
  switch(maskX){
    case 15:
      switch(maskY){
        case 0b1111:
          __builtin_mma_pmxvbf16ger2pp(acc, reinterpret_cast<Packet16uc>(a.m_val), reinterpret_cast<Packet16uc>(b.m_val), 0b1111, 0b1111, 0b11);
          break;
        case 0b0011:
...
EIGEN_STRONG_INLINE Packet8bf loadRhsBfloat16(const bfloat16* baseB, Index strideB, int i, int k)
{
  ...
  Packet8bf rhs1 = pload<Packet8bf>(indexB);  
  if(zero){
...
template<typename Index, int num_acc, int num_packets, bool zero, bool rhs_extra_cols, bool lhs_extra_rows>
...
template<typename Index, const int num_acc, const int standard_block_size, const int num_packets, bool rhsExtraCols = false, bool lhsExtraRows = false>
